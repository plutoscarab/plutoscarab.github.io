...
namespace PlutoScarab
{
    ...
    class Program
    {
        ...
        static BigInteger D(int i, int n)
        {
            if (i == 0)
                return Factorial(n);

            return D(i - 1, n + 1) - D(i - 1, n);
        }

        static BigInteger S(int i, int n)
        {
            if (i == 0)
                return Factorial(n);

            return S(i - 1, n + 1) + S(i - 1, n);
        }

        static bool TryComputeGCF(IEnumerable<BigInteger> ps, IEnumerable<BigInteger> qs, int digits, out string s, out int terms)
        {
            s = default;
            terms = default;
            var pterms = 0;
