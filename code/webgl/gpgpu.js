 ...
 */
  ...
  */
 function buildRasterProgram(gl,fragment,defines,header) {
     var program = getDefaultRasterProgram(gl);

     // Begin definition of shader source by specifying float precision
     var source = "";
     //source += "precision mediump float;\n"
     source += "precision highp   float;\n"   
      
     // Include any header if present
     source += "\n" + (header || "") + "\n";
  
     // Compute predefind variables;
     // These are more or less exposes as compile-time "curried"
     // arguments by the GPGPU API
     defines = defines || {};
     ...
     function callable_program(inputs, output) {
         if (callable_program.arguments.length === 1) {
           ...
           /*
           console.log(
             `Caution: GPU program called with one argument. 
             Single input being treated as arguments list. 
             Not as a render target. 
             If this is not the case, 
             missing variable errors will ensue.`);
             */
         }
     ...
     for (var key in gl) {
         ...
         if (typeof it==='function') { 
             ...
             var safe = function(...args) {
                 it.apply(it, [args]); check_error(gl);
             };
             gl['key']=safe;
         }
     }
 ...
 function applyProgram(program, inputs, output) {
     ...
     for (var key in inputs) if (inputs.hasOwnProperty(key)) {
         ...
         else if (param.mode==='variable'||param.mode==='matrix') 
             param.set(inputs[key]);
         else {
             throw new Error('Unrecognized parameter mode'+param+' '+param.mode);
         }
     }
